	...
	<div class="mt-3 container">
		<div class="bg-dark-2 blockquote bq-primary text-light d-flex align-items-center">
			...
			<div class="controls-order-history ml-5 d-flex align-items-center">
				<div class="btn-group ml-4 custom-checkbox-button-group" role="group" aria-label="Basic example">
					<button type="button" class="btn btn-primary btn-md active">Deposit</button>
					<button type="button" class="btn btn-primary btn-md">Withdraw</button>
					<button type="button" class="btn btn-primary btn-md">Fee</button>
				</div>
			</div>
		</div>
		<table id="transactionHistory" width="100%" class="table table-striped"></table>
	</div>
</div>
@endsection
...
$('#transactionHistory_wrapper .dataTables_filter').find('label').each(function () {
	$(this).parent().append($(this).children());
});
$('#transactionHistory_wrapper .dataTables_filter').find('input').each(function () {
	$('input').attr("placeholder", "Search");
	$('input').removeClass('form-control-sm');
});
$('#transactionHistory_wrapper .dataTables_filter').addClass('md-form');
$('#transactionHistory_wrapper .dataTables_filter').find('label').remove();
