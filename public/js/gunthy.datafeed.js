class GunthyDatafeed {
    constructor(options) {
        this.binanceHost = options.proxy || 'https://exchange.gunthy.org/trading_view/'
        this.debug = options.debug || false
    }

    binanceServerTime() {
        return fetch(this.binanceHost + 'serverTime').then(res => {
            return res.json()
        }).then(json => {
    ...
    binanceSymbols() {
        return fetch(this.binanceHost + 'exchangeInfo').then(res => {
            return res.json()
        }).then(json => {
            return json.symbols
        })
    ...
    binanceKlines(symbol, interval, startTime, endTime, limit) {
        const url = this.binanceHost + 'klines' +
